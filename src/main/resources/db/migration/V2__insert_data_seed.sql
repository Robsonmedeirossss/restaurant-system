...
INSERT INTO products(category_id, name, description, price, preparation_minutes)
SELECT category.id, 'X-Bacon', 'Hamburguer com bacon e queijo cheddar', 26.99, 25
FROM categories_products category WHERE category.name = 'Hamburguers';

INSERT INTO products(category_id, name, description, price, preparation_minutes)
SELECT category.id, 'Pizza Calabresa', 'Pizza com calabresa e cebola', 40.50, 35
FROM categories_products category WHERE category.name = 'Pizzas';

INSERT INTO products(category_id, name, description, price, preparation_minutes)
SELECT category.id, 'Suco de Laranja', 'Suco de laranja 250ml', 10.00, 10
